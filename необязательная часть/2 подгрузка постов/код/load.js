'use strict';
//Сделаем побольше размеры окну просмотра: .scrollCheck{}


// 1. Получите ссылку на .scrollCheck
let scrollCheck = document.querySelector('.scrollCheck');
//console.log(scrollCheck.getBoundingClientRect()); //Выводит объект

// top - расстояние от верхней границы блока до верхней граниы вьюпорта (окна просмотра)
// bottom - расстояние от верхней границы самого окна до нижней границы блока (высота+top)
// left - отступ от левого края окна просмотра до левого края блока
// right - расстояние от левого края окна просмотра до правого края блока
// x - от верхнего левого угла окна просмотра до верхнего левого угла блока = Left
// y - расстояние от верхней границы блока до верхней граниы вьюпорта = top
// window.innerHeight = посмотреть размер окна
// document.body.offsetHeight = полная высота body с учетом всех марджинов и прочих
/**
...
function init() {
    ...
    // init.
    // Если высота окна = высоте body, то вызываем подгрузку постов
...
document.addEventListener('scroll', function(event) {
    // если отступ от верхнего края до блока <= Высоте окна,
    // это значит что мы до самой нижней границы домотали
...
function getPostMarkup(counter) {
    ...
    плюс counter
    Можно еще использовать date.prototype.getTime() - см.mdn
    */
    let num =  new Date().getTime() + counter;
    return `<div class="post">
        <img src="https://picsum.photos/200/300?random=${num}"> //чтобы не было повторяющихся чисел
